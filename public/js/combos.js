$(document).ready(function () {
    ...
    // * Params: id
    // * type: string
    // * Details: return info of spesific activity and option or options (HTML) it depends on type activity (LIBRE or MULTIPLE)
    // */
            ...
            <form class="form-actividad-combo" >
              <div class="form-group">
                <select class="form-control select-info" name="select";> 
                  ...
                  <input type="hidden" name="actividad_combo_id" value="${
										actividad.id
									}">
              </div>
            </form>
  ...
  `);
            ...
            let optionSelecionado = document.querySelector(
                "option[data-horariotext]"
            );
            let textoption = optionSelecionado.getAttribute(
                "data-horariotext"
            ); /* text de DB */
    ...
    $("#mismodia").on("click", function () {
        $("#mismodia").prop("checked") == true ?
            $("#mismodia").val("1") :
            $("#mismodia").val("0");
    });
    ...
    function printInfoCombo(infoCombo, infoPrecios) {
        ...
        $("#balanceG").val( Math.round(Number(infoCombo.balance)));
        $("#precioG").val(Math.round(Number(infoCombo.precio)));
        $("#tipoactividades_id").val(infoCombo.tipoactividades_id);
        $("#anticipoId").val(infoCombo.anticipo_id);
        infoCombo.mismo_dia == 1 ?
            $("#mismo_dia").prop("checked", true) :
        ...
        infoPrecios.forEach(infoPrecio => {
            ...
            infoPrecio.restriccion == 1 ?
                $(`#formPerson${infoPrecio.persona_id}`)[0][9].setAttribute(
                    "checked",
                    "true"
                ) :
                $(`#formPerson${infoPrecio.persona_id}`)[0][9].setAttribute(
            ...
            infoPrecio.acompanante == 1 ?
                $(`#formPerson${infoPrecio.persona_id}`)[0][10].setAttribute(
                    "checked",
                    "true"
                ) :
                $(`#formPerson${infoPrecio.persona_id}`)[0][10].setAttribute(
